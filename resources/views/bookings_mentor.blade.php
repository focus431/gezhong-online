	...
	function generateBookingRows(data, tbody, currentPage) {
		...
		if (Array.isArray(data.classSchedules.data)) {
			data.classSchedules.data.forEach((schedule, index) => {
				...
				aName.href = 'profile';
				const mentee = data.users.find(users => users.id === schedule.mentee_id);
				...
				console.log('data.classSchedules.data:', data.classSchedules.data);
				...
				const googlemeetId = data.user; // 修改這裡使用 data.user

				if (googlemeetId.id) {
					actionBtn.setAttribute('data-meeting-url', googlemeetId.google_meet_code);
				} else {
